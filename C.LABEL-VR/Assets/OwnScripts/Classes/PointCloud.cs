...
public class PointCloud
{
    ...
    public PointCloud(HDF5Addon.Lidar_Daimler hdf5Container_inp)
    {
        ...
        Util.CustomAttributesContainer[,] attributeTable = new Util.CustomAttributesContainer[rows, cols];

        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                Util.CustomAttributesContainer attribute = new Util.CustomAttributesContainer
                {
                    _tableIndex = new Util.Tuple<int, int>(i, j),
    ...
    private void CalculateGroundPoints(Util.CustomAttributesContainer[,] pointTable_inp, ref List<GameObject> points_out)
    {
        ...
        for (int i = 0; i < cols; i++)
        {
            ...
            verticalLine[rows] = new Util.CustomAttributesContainer();
    ...
    private void ProcessVerticalLine(Util.CustomAttributesContainer[] verticalLine_inp, ref List<GameObject> points_out)
    {
        ...
        for (int i = verticalLine_inp.Length - 1; i > 0; i--)
        {
            int prePoint = i;
            int curPoint = i - 1;
            ...
            float alpha = Mathf.Rad2Deg * Mathf.Asin(height / distance);
        ...
        {
            //found
            for (int i = verticalLine_inp.Length-2; i > 1; i--)
            {
                if (i >= thresholdPoint)
                {
                    //is ground
